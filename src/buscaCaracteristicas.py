...
def removerEstrelas(imagemFinal, classificacoes, objetos, classeEstrela, imagemPontos):
    for i in range(len(objetos)):
        if objetos[i]['classificacaoGerada'] == classeEstrela and objetos[i]['indice'] != 254:  # REMOVER OBJETOS CLASSIFICADOS COMO 'classeEstrela'
            for pos in objetos[i]["area"]:
                imagemFinal[pos[0]][pos[1]] = 0
...
# labels = cor do objeto (rótulo)
# indice = numero que o objeto representa
# imagemRotulado = imagem segmentada pelo watershed
...
def mapaDistancia(imagemLimiarizada):
    ...
    cv2.normalize(imagemDistancia, imagemDistancia, 0, 255.0, cv2.NORM_MINMAX) #Normalizar os valores atribuidos para ficar dentro do range(0 até 255.0)
...
# Retorna uma imagem(8bits) contendo apenas a borda em valor 255.
def encontrarBorda(imagemLimiarizada):
    ...
    for y in range(altura):
        for x in range(largura):
            if imagemLimiarizada[y][x] != 0:
                if y > 0 and y < imagemLimiarizada.shape[0]-2 and x > 0 and x < imagemLimiarizada.shape[1]-2: #pula os pixels da borda da imagem para fazer a verificação abaixo com uma matriz 9x9
